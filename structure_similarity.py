...
def write_pyd(input_folder_path, output_folder_path):
    for pdb_file in tqdm(glob.iglob(input_folder_path + "/*.pdb")):
        print(pdb_file)
        ...
        with open(output_folder_path + f"/{save_name}.pyd", 'wb') as f:
            pickle.dump(record, f)




task = ["function_0", "function_1", "process_0", "process_1", "component_0", "component_1"]

    
    
def process_task(task_name):
    
    folder_name = "evaluate_candidate_sequence"
    
    
    input_folder_path = os.path.join(folder_name, task_name, "pdb")
    output_folder_path = os.path.join(folder_name, task_name, "structure_embedding")
    

    
    if not os.path.exists(output_folder_path):
        os.makedirs(output_folder_path)    
    write_pyd(input_folder_path, output_folder_path)
...
with ThreadPoolExecutor(max_workers=len(task)) as executor:
    executor.map(process_task, task) 