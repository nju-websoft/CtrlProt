...
class Structure_Evaluate():
    ...
    def get_plddt_scores(self, task_name, sequences,num = -1):
        results = []
        tmscores = []
        rmsds = []
        temp_sequences = sequences[ : num]
        
        db_path = os.path.join("/data1/anonymity/foldseek", task_name)
        for test_protein in temp_sequences:
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    tmscore, rmsd = self.get_foldseek_result(pdb_content, db_path)
...
def main():
    ...
    args.dataset_path = "mlpo_model_result"

    task_name = ["function_0", "function_1"]


    for task  in task_name:
        folder_path = os.path.join(args.dataset_path,  task)
        ...
        with open(result_path,"w") as file:
            for subfolder in tqdm(subfolder_list):
                ...
                for file_path in tqdm(file_list):   
                    ...
                    plddt_scores, tmscores, rmsds = structure_evaluate.get_plddt_scores(task, sequences, num = 10)
                    ...
                    avg_tmscore = sum(tmscores)/len(tmscores) if len(tmscores)!=0 else 0
                    avg_rmsd = sum(rmsds)/len(rmsds) if len(rmsds)!=0 else 0
                    
                    avg_tmscore = round(avg_tmscore,4)
                    avg_rmsd = round(avg_rmsd,4)
                    
                    file.write(f"{file_name}, function:{function_score}, process:{process_score}, component:{component_score}, plddt:{avg_plddt_score}, tmscore:{avg_tmscore}, rmsd:{avg_rmsd}\n")
